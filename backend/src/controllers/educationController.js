...
// Helper functions
const calculateLearningStats = async (userId) => {
  const totalTopics = await Topic.countDocuments();
  const completedTopics = Math.min(
    await UserTopicProgress.countDocuments({ userId, completed: true }),
    totalTopics,
  );
  ...
  for (const cat of categories) {
    ...
    const completedInCat = await UserTopicProgress.countDocuments({
      userId,
      categoryId: cat._id,
      completed: true,
    });
    ...
    // Estimate 10 minutes per topic (5 topics ≈ 0.83 hours, but let's use a reasonable estimate)
    totalLearningHours += Math.round((completedInCat * 10) / 60);
  }
  ...
  const skillScore = Math.min(
    100,
    Math.round(completedTopics * 2 + completedCategories * 10),
  );

  return {
    totalTopics: totalTopics || 0,
    completedTopics: completedTopics || 0,
    completedCategories: completedCategories || 0,
    overallProgress: overallProgress || 0,
    coursesCompleted: completedCategories || 0,
    certificates: completedCategories || 0,
    learningHours: totalLearningHours || 0,
  ...
  try {
    ...
    const categoriesWithProgress = await Promise.all(
      categories.map(async (cat) => {
        ...
        const completedCount = await UserTopicProgress.countDocuments({
          userId,
          categoryId: cat._id,
          completed: true,
        });
        const progress =
          topicCount > 0
            ? Math.min(100, Math.round((completedCount / topicCount) * 100))
            : 0;
        return { ...cat.toObject(), topicCount, completedCount, progress };
      }),
    );
    // Get stats
    const stats = await calculateLearningStats(userId);
...
export const getCategory = async (req, res) => {
  try {
    ...
    const topicsWithProgress = await Promise.all(
      topics.map(async (topic) => {
        const progress = userId
          ? await UserTopicProgress.findOne({ userId, topicId: topic._id })
          : null;
        return {
          ...topic.toObject(),
    ...
    const quizQuestions = await Quiz.find({ category: category.name }).limit(
      10,
    );
    res
      .status(200)
...
export const getTopic = async (req, res) => {
  try {
    ...
    let progress = userId
      ? await UserTopicProgress.findOne({ userId, topicId: id })
      : null;
    if (!progress && userId) {
    ...
    res
      .status(200)
      .json({ success: true, data: { topic, userProgress: progress } });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: "Internal server error" });
  }
...
export const markTopicComplete = async (req, res) => {
  try {
    ...
    if (!progress) {
      ...
      progress = await UserTopicProgress.findByIdAndUpdate(
        progress._id,
    ...
    if (topicCount === completedCount) {
      ...
      if (!existingCert) {
        await Certificate.create({
          ...
          title: `${category.name} Completion Certificate`,
          description: `Completed ${category.name}`,
        });
      }
    }
    res
...
export const submitQuiz = async (req, res) => {
  try {
    ...
    const isCorrect = Number(selectedAnswer) === Number(quiz.correctAnswer);
...
const getLeaderboardData = async () => {
  const allProgress = await UserTopicProgress.aggregate([
    {
      $group: {
        _id: "$userId",
        completedTopics: { $sum: { $cond: ["$completed", 1, 0] } },
      },
    },
  ...
  return allProgress.map((p, i) => ({
    rank: i + 1,
    name:
      users.find((u) => u._id.toString() === p._id.toString())?.fullName ||
      "Anonymous",
...
const getContinueLearningData = async (userId) => {
  // First get all topics with progress
  const categories = await CourseCategory.find().sort({ order: 1 });
  for (const cat of categories) {
    const topics = await Topic.find({ category: cat._id }).sort({ order: 1 });
  ...
  // If all topics are completed, return null to trigger the "all completed" message
  return null;
};
