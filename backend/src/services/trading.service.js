...
export const executeSellOrder = async (userId, { positionId, quantity }) => {
  ...
  try {
    ...
    if (position.orderType === 'BUY') {
      // For BUY: Profit/Loss = (Exit Price - Entry Price) × Quantity
      pnl = (exitPrice - position.entryPrice) * quantity;
    } else if (position.orderType === 'SELL') {
      // For SELL (Short): Profit/Loss = (Entry Price - Exit Price) × Quantity
      pnl = (position.entryPrice - exitPrice) * quantity;
    } else {
      pnl = 0;
    }
    ...
    // Handle position
    if (position.quantity === quantity) {
      // Close entire position - freeze the exit price
      position.status = 'CLOSED';
      position.exitPrice = exitPrice;
      await position.save({ session });
    } else {
      // Partial sell, reduce quantity and update current price
