  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (type === "buy") {
        ...
        toast.success("Sell order functionality will be implemented from the Positions page!");
      }
      navigate("/orders");
    } catch (error) {
              ...
              <div className="bg-[#050816] rounded-xl p-4 border border-white/5">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <div className="text-white font-semibold">₹{tradeData.entryPrice?.toFixed(2)}</div>
                  </div>
                  {tradeData.pattern && (
                    <div>
                      <div className="text-[#B8C0D4]">Pattern</div>
                      <div className="text-[#32CD32] font-semibold">{tradeData.pattern}</div>
                    </div>
                  )}
                  {tradeData.support && (
                    <div>
                      <div className="text-[#B8C0D4]">Support</div>
                      <div className="text-green-400 font-semibold">₹{tradeData.support.toFixed(2)}</div>
                    </div>
                  )}
                  {tradeData.resistance && (
                    <div>
                      <div className="text-[#B8C0D4]">Resistance</div>
                      <div className="text-red-400 font-semibold">₹{tradeData.resistance.toFixed(2)}</div>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-[#050816] rounded-xl p-4 border border-white/5">
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span className="text-white font-semibold">
                    ₹{(tradeData.entryPrice * quantity).toFixed(2)}
                  </span>
                </div>
              </div>
